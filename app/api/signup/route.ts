...
export async function POST(req: NextRequest) {
  ...
  if (!name || !email || !password) {
    return NextResponse.json({ message: '빈칸을 모두 입력해주세요.' })
  }
  if (!emailRegex.test(email)) {
    return NextResponse.json({ message: '잘못된 이메일 형식입니다.' })
  }
  if (name.length > 10) {
    return NextResponse.json({ message: '이름은 10자 이하로 입력해주세요.' })
  }
  ...
  try {
    ...
    if (existedUser) {
      return NextResponse.json(
        { message: '이미 존재하는 이메일입니다.', status: '409' },
        { status: 409 },
      )
    }
